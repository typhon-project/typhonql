...
TestExecuter initTest(void(PolystoreInstance, bool) setup, str host, str port, str user, str password, Log log = NO_LOG()) {
	...
	list[CommandResult](Request req, list[str] columnNames, list[str] columnTypes, list[list[str]] vs) 
        ...
        checkRequest(req);
	    ses = newSession(connections); 
		result = runPreparedUpdateInTest(req, columnNames, columnTypes, vs, sch, ses, log);
		ses.done();
		return result;
	};
...
list[CommandResult] runPreparedUpdateInTest(Request req, list[str] columnNames, list[str] columnTypes, list[list[str]] vs, Schema s, Session session, Log log) {
	session.setStage0(columnNames, columnTypes, vs);
	return runPrepared2(req, s, session, log = log);
}
