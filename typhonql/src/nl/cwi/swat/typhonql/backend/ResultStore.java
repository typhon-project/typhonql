...
public class ResultStore {
	...
	private List<List<String>> toValues(List<Field> fields, List<List<Record>> rs) {
		List<List<String>> vs = new ArrayList<>();
		for (List<Record> records : rs) {
			List<String> os = new ArrayList<>();
			for (Record r : records) {
				for (Field f : fields) {
					String serializedObject = ResultTable.serialize(f.getType(), r.getObject(f));
					os.add(serializedObject);
				}
			}
			vs.add(os);
		}
