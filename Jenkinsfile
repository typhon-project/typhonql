node {
  ...
  if (env.BRANCH_NAME == "master") {
    env.MAVEN_OPTS= "-Dmaven.repo.local=.m2-ql-master -Djava.awt.headless=true"
  }
  else {
    env.MAVEN_OPTS= "-Dmaven.repo.local=.m2-ql-dev -Djava.awt.headless=true"
  }
    try{
    notifyBuild()
	properties([
    ...
    stage('Build bundle') {
	    configFileProvider(
            ...
            sh 'cd typhonql-bundler && mvn -X -U -B -gs $MAVEN_SETTINGS clean install'
	    }
    }
